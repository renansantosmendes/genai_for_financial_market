...
def gerar_serie_com_variacao(model, sequencia_inicial, scaler, seq_len,
    ...
    """
    Gera uma série temporal com variações estocásticas
    ...
    with torch.no_grad():
        for step in range(num_steps):
            ...
            # Adicionar ruído estocástico
            if adicionar_ruido:
                # Ruído proporcional ao desvio padrão dos dados
                ruido = np.random.normal(0, 0.05 * temperatura, proximo_valor.shape)
...
def gerar_multiplas_series(model, dados_base, scaler, seq_len,
    ...
    """
    Gera múltiplas séries temporais com diferentes configurações
    ...
    """
    print(f"\n{'=' * 60}")
    print(f"GERANDO {num_series} SÉRIES TEMPORAIS")
    print(f"{'=' * 60}")

    series_geradas = []
    configs = []

    # Se não especificado, gera temperaturas aleatórias
    if temperaturas is None:
        temperaturas = np.random.uniform(0.7, 1.3, num_series)
    ...
    for i in range(num_series):
        ...
        if usar_diferentes_seeds and len(dados_base) > seq_len * 2:
            # Usar diferentes partes dos dados históricos
            ...
            # Usar últimos seq_len pontos
            sequencia_inicial = dados_base[-seq_len:]

        # Configuração desta série
        temp = temperaturas[i] if i < len(temperaturas) else 1.0
        adicionar_ruido = np.random.rand() > 0.3  # 70% com ruído

        config = {
            'temperatura': temp,
            'ruido': adicionar_ruido,
        ...
        serie = gerar_serie_com_variacao(
            ...
            num_steps=num_steps,
            temperatura=temp,
            adicionar_ruido=adicionar_ruido
        )

        series_geradas.append(serie)
        print(f"  Temperatura: {temp:.2f}, Ruído: {adicionar_ruido}")
...
def exibir_estatisticas_gerais(series_geradas, configs):
    ...
    print(f"\n{'=' * 70}")
    print(f"ESTATÍSTICAS COMPARATIVAS")
    print(f"{'=' * 70}")
    ...
    for i, (serie, config) in enumerate(zip(series_geradas, configs), 1):
        ...
        print(f"\n📊 Série {i}:")
        print(f"   Temperatura: {config['temperatura']:.3f}")
        print(f"   Ruído: {'Sim' if config['ruido'] else 'Não'}")
