...
def gerar_multiplas_series(model, dados_base, scaler, seq_len,
    ...
    """
    print(f"\n{'=' * 60}")
    print(f"GERANDO {num_series} SÉRIES TEMPORAIS NÃO-ESTACIONÁRIAS")
    print(f"{'=' * 60}")
    ...
    for i in range(num_series):
        ...
        if com_diversidade:
            # Diferentes comportamentos para cada série
            comportamentos = [
                {'momentum': 0.3, 'vol_crescente': False, 'shock': True, 'ruido': True},  # Tendência alta com shocks
                {'momentum': -0.3, 'vol_crescente': False, 'shock': True, 'ruido': True},  # Tendência baixa com shocks
                {'momentum': 0.1, 'vol_crescente': True, 'shock': False, 'ruido': True},  # Vol crescente
                {'momentum': 0, 'vol_crescente': False, 'shock': True, 'ruido': True},  # Apenas shocks
                {'momentum': 0.5, 'vol_crescente': False, 'shock': False, 'ruido': True},  # Forte tendência
                {'momentum': -0.2, 'vol_crescente': True, 'shock': True, 'ruido': True},  # Mix completo
                {'momentum': 0.2, 'vol_crescente': False, 'shock': True, 'ruido': True},  # Tendência moderada
                {'momentum': 0, 'vol_crescente': True, 'shock': False, 'ruido': True},  # Vol crescente sem tendência
                {'momentum': -0.4, 'vol_crescente': False, 'shock': True, 'ruido': True},  # Tendência forte baixa
            ]

            comportamento = comportamentos[i % len(comportamentos)]
        else:
            # Comportamento padrão (pode convergir)
            comportamento = {'momentum': 0, 'vol_crescente': False, 'shock': False, 'ruido': True}
